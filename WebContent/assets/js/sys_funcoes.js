...
function init_botao_buscar_endereco(){
	try{
		$('#search_endereco').val("Localizar endere�o");
...
function limitChars(textid, limit, infodiv)
{
    ...
    if(textlength > limit)
    {
        $('#' + infodiv).html('Voce n�o pode escrever mais que '+limit+' caracteres!');
        $('#'+textid).val(text.substr(0,limit));
        return false;
    }
    else
    {
        $('#' + infodiv).html('Voc� possui '+ (limit - textlength) +' caracteres restante.');
        return true;
    }
}
